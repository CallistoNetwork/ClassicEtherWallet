...
const ChangeNow = function() {
  ...
  this.exchangeLink = (from = "btc", to = "etc", amount = 1) =>
    `https://changenow.io/exchange?amount=${amount}&from=${from}&link_id=${
      this.linkId
    }&to=${to}`;
  this.availableCoins = [];
  ...
  this.estimateConversion = async function(
    to = "etc",
    from = "btc",
    amount = 1
  ) {
    ...
    if (this.conversionRatios.hasOwnProperty(from + "/" + to)) {
      ...
      return (
        result &&
        this.availableCoins.find(coin => coin.ticker.toLowerCase() === to)
      );
    }
  };
  ...
  this.exchangeAmount = async function(amount, from, to) {
    ...
    const result = await ajaxReq.http.get(
      this.uri_base + `/exchange-amount/${amount}/${from}/${to}`
    );

    if (result.statusText.toUpperCase() === "OK") {
      const idx = this.availableCoins.findIndex(
        coin => coin.ticker.toLowerCase() === to
      );
  ...
  this.minAmount = async function(from, to) {
    const result = await ajaxReq.http.get(
      this.uri_base + `/min-amount/${from.toLowerCase()}/${to.toLowerCase()}`
    );
  ...
  this.openOrder = async function(order) {
    /*
         ...
         */
    const result = await ajaxReq.http.post(
      this.uri_base + `/transactions`,
