...
var addWalletCtrl = function($scope, $sce) {
    ...
    $scope.onPrivKeyChange = function() {
        ...
        $scope.requirePPass =
            $scope.manualprivkey.length == 128 ||
            $scope.manualprivkey.length == 132;
    ...
    $scope.onMnemonicChange = function() {
        $scope.addWalletStats = "";
        $scope.showBtnUnlock = $scope.showDPaths = hd.bip39.validateMnemonic(
            $scope.manualmnemonic
        );
    };
    ...
    $scope.decryptWallet = function() {
        ...
        try {
            if ($scope.walletType == "pasteprivkey" && $scope.requirePPass) {
                ...
                $scope.HDWallet.hdk = hd.HDKey.fromMasterSeed(
                    hd.bip39.mnemonicToSeed($scope.manualmnemonic.trim())
                );
                $scope.HDWallet.numWallets = 0;
                $scope.setHDAddresses(
                    $scope.HDWallet.numWallets,
    ...
    $scope.newWalletChange = function(varStatus, shwbtn) {
        if (
            $scope.addAccount.nickName != "" &&
            $scope.nickNames.indexOf($scope.addAccount.nickName) == -1 &&
            $scope.addAccount.password.length > 8
        )
            $scope[shwbtn] = true;
    ...
    $scope.watchOnlyChange = function() {
        if (
            $scope.addAccount.address != "" &&
            $scope.addAccount.nickName != "" &&
            $scope.nickNames.indexOf($scope.addAccount.nickName) == -1 &&
            ethFuncs.validateEtherAddress($scope.addAccount.address)
        )
        ...
        else if ($scope.nickNames.indexOf($scope.addAccount.nickName) !== -1)
            $scope.notifier.danger(globalFuncs.errorMsgs[13]);
    };
    $scope.addWatchOnly = function() {
        if ($scope.nickNames.indexOf($scope.addAccount.nickName) !== -1) {
            ...
            $scope.nickNames.indexOf(
                ethUtil.toChecksumAddress($scope.addAccount.address)
            ) !== -1
        ) {
            $scope.notifier.danger(globalFuncs.errorMsgs[16]);
            return;
        }
        cxFuncs.addWatchOnlyAddress(
            $scope.addAccount.address,
            $scope.addAccount.nickName,
            function() {
                if (chrome.runtime.lastError) {
                    ...
                    $scope.notifier.info(
                        globalFuncs.successMsgs[3] + $scope.addAccount.address
                    );
                    $scope.setNickNames();
                }
                $scope.$apply();
    ...
    $scope.addWalletToStorage = function() {
        ...
        cxFuncs.addWalletToStorage(
            $scope.addAccount.address,
            $scope.addAccount.encStr,
            $scope.addAccount.nickName,
            function() {
                if (chrome.runtime.lastError) {
                    $scope.notifier.danger(chrome.runtime.lastError.message);
    ...
    $scope.setBalance = function() {
        ajaxReq.getBalance($scope.wallet.getAddressString(), function(data) {
            if (data.error) {
                ...
                $scope.etherBalance = etherUnits.toEther(
                    data.data.balance,
                    "wei"
                );
                _coinPrice().then(function(data) {
                    ...
                    $scope.eurBalance = etherUnits.toFiat(
                        $scope.etherBalance,
                        "ether",
                        data.eur
                    );
                    $scope.btcBalance = etherUnits.toFiat(
                        $scope.etherBalance,
                        "ether",
                        data.btc
                    );
