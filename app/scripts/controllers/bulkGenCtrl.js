...
var bulkGenCtrl = function($scope) {
  ...
  var network =
    globalFuncs.urlGet("network") == null ? "" : globalFuncs.urlGet("network");

  $scope.genWallets = function() {
    if ($scope.amount == "" || $scope.amount != parseInt($scope.amount, 10))
      alert(globalFuncs.errorMsgs[0]);
    else {
      $scope.wallets = [];
      var csv = "";
      var txt = "";
      ...
      for (var i = 0; i < $scope.amount; i++) {
        ...
        csv +=
          tWallet.getChecksumAddressString() +
          "," +
          tWallet.getPrivateKeyString() +
          "\n";
        txt +=
          tWallet.getChecksumAddressString() +
          "\t" +
      ...
      $scope.bJSON = globalFuncs.getBlob(
        "text/json;charset=UTF-8",
        JSON.stringify($scope.jsonWallets)
      );
