...
var contractsCtrl = function ($scope, $sce, $rootScope, walletService) {
    ...
    const initContract = {
        address: Validator.isValidAddress(globalFuncs.urlGet('address')) ? globalFuncs.urlGet('address') : '',
        abi: [],
        functions: [],
        selectedFunc: null,
   ...
   */
    ...
    $scope.initContract = function () {
        try {
            if (!Validator.isValidAddress($scope.contract.address)) throw globalFuncs.errorMsgs[5];
            else if (!Validator.isJSON($scope.contract.abi)) throw globalFuncs.errorMsgs[26];
