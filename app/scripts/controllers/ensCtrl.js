...
var ensCtrl = function($scope, $sce, $rootScope, walletService) {
  ...
  $scope.objENS = {
    ...
    tld:
      nodes.tldTypes[
        ajaxReq.type.substring(ajaxReq.type.length - 3, ajaxReq.type.length)
      ],
    txSent: false
  };
        ...
        if (data.error) $scope.notifier.danger(data.msg);
          ...
          switch ($scope.objENS.status) {
            case $scope.ensModes.owned:
              ENS.getOwner($scope.objENS.name + $scope.objENS.tld, function(
                data
              ) {
              ...
              ENS.getAddress($scope.objENS.name + $scope.objENS.tld, function(
                data
              ) {
  ...
  $scope.onLongStringChanged = function() {
    try {
      ...
      if (tObj.value)
        $scope.objENS.bidValue = Number(etherUnits.toEther(tObj.value, "wei"));
  ...
  $scope.getRevealTime = function() {
    if ($scope.objENS && $scope.objENS.registrationDate)
      return new Date($scope.objENS.registrationDate - 48 * 60 * 60 * 1000);
    return new Date().toString();
  };
