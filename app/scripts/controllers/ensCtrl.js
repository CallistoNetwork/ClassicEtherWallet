...
var ensCtrl = function($scope, $sce, $rootScope, walletService) {
    ...
    $scope.ensConfirmModalModal = new Modal(
        document.getElementById("ensConfirmModal")
    );
    $scope.ensFinalizeModal = new Modal(
        document.getElementById("ensFinalizeConfirm")
    );
    ...
    $scope.objENS = {
        ...
        tld:
            nodes.tldTypes[
                ajaxReq.type.substring(
                    ajaxReq.type.length - 3,
                    ajaxReq.type.length
                )
            ],
        txSent: false
    ...
    $scope.checkName = function() {
        ...
        if (
            $scope.haveNotAlreadyCheckedLength &&
            ($scope.objENS.name.length == 128 ||
                $scope.objENS.name.length == 132 ||
                $scope.objENS.name.length == 64 ||
                $scope.objENS.name.length == 66)
        ) {
            $scope.notifier.danger(
                "That looks an awful lot like a private key. Are you sure you would like to check if this name is available on the ENS network? If so, click `Check`. If it is your private key, click refresh & try again."
            );
            $scope.haveNotAlreadyCheckedLength = false;
        } else if (
            $scope.Validator.isValidENSName($scope.objENS.name) &&
            $scope.objENS.name.indexOf(".") == -1
        ) {
            $scope.objENS.name = ens.normalise($scope.objENS.name);
            $scope.objENS.namehash = ens.getNameHash(
                $scope.objENS.name + $scope.objENS.tld
            );
                ...
                if (data.error) $scope.notifier.danger(data.msg);
                    ...
                    switch ($scope.objENS.status) {
                        case $scope.ensModes.owned:
                            ...
                            ENS.getDeedOwner($scope.objENS.deed, function(
                                data
                            ) {
                        ...
                        case $scope.ensModes.notAvailable:
                            ENS.getAllowedTime($scope.objENS.name, function(
                                data
                            ) {
                                $scope.objENS.allowedTime = data.data;
                                clearInterval($scope.objENS.timer);
                                $scope.objENS.timer = setInterval(
                                    () => timeRem($scope.objENS.allowedTime),
                                    1000
                                );
                            });
                            break;
                        case $scope.ensModes.auction:
                            clearInterval($scope.objENS.timer);
                            $scope.objENS.timer = setInterval(
                                () => timeRem($scope.objENS.registrationDate),
                                1000
                            );
                            break;
                        case $scope.ensModes.reveal:
                            ...
                            $scope.objENS.secret = "";
                            $scope.objENS.highestBid = etherUnits.toEther(
                                $scope.objENS.highestBid.toString(),
                                "wei"
                            );
                            clearInterval($scope.objENS.timer);
                            $scope.objENS.timer = setInterval(
    ...
    $scope.onLongStringChanged = function() {
        try {
            ...
            if (tObj.value)
                $scope.objENS.bidValue = Number(
                    etherUnits.toEther(tObj.value, "wei")
                );
            ...
            if (tObj.name && ens.normalise(tObj.name) != $scope.objENS.name) {
                ...
                tObj.owner &&
                tObj.owner != $scope.wallet.getAddressString()
            ) {
                // check owner = bidder
                $scope.notifier.danger(globalFuncs.errorMsgs[33]);
            } else {
                //estimate gas to see if it would not work
    ...
    $scope.openAndBidAuction = function() {
        ...
        _objENS.registrationDate.setDate(
            _objENS.registrationDate.getDate() + 5
        );
        getShaBid($scope.bidObject, function(isError, data) {
            if (isError) $scope.notifier.danger(data);
            else {
                var bidHash = data;
                $scope.tx.data = ENS.getStartAndBidAuctionData(
                    $scope.objENS.name,
                    bidHash
                );
    ...
    $scope.revealBid = function() {
        ...
        var _objENS = $scope.objENS;
        ajaxReq.getTransactionData($scope.wallet.getAddressString(), function(
            data
        ) {
            if (data.error) $scope.notifier.danger(data.msg);
            data = data.data;
            $scope.tx.to = ENS.getAuctionAddress();
            $scope.tx.data = ENS.getRevealBidData(
                _objENS.name,
                etherUnits.toWei(_objENS.bidValue, "ether"),
                _objENS.secret
            );
            $scope.tx.value = 0;
            var txData = uiFuncs.getTxData($scope);
    ...
    $scope.getRevealTime = function() {
        if ($scope.objENS && $scope.objENS.registrationDate)
            return new Date(
                $scope.objENS.registrationDate - 48 * 60 * 60 * 1000
            );
        return new Date().toString();
    };
        ...
        uiFuncs.sendTx(signedTx, function(resp) {
            if (!resp.isError) {
                var emailLink =
                    ...
                    $scope.objENS.name +
                    "%0A%20timeRemaining%3A%20" +
                    $scope.getRevealTime().toString() +
                    "%0A%20revealDate%3A%20" +
                    $scope.objENS.registrationDate.toString() +
                    "%0A%20timer%3A%20" +
                    $scope.objENS.timer +
                    "%0A%20txSent%3A%20" +
                    $scope.objENS.txSent +
                    "%0A%20to%3A%20" +
                    $scope.tx.to +
                    "%0A%20from%20address%3A%20" +
                    $scope.wallet.getAddressString() +
                    "%0A%20data%3A%20" +
                    $scope.tx.data +
                    "%0A%20value%3A%20" +
                    $scope.tx.value +
                    '" target="_blank" rel="noopener">Confused? Email Us.</a>';
                var bExStr =
    ...
    $scope.generateTx = function() {
        try {
            ...
            if (!$scope.Validator.isValidENSName(_objENS.name))
                ...
                !$scope.Validator.isPositiveNumber(_objENS.bidValue) ||
                _objENS.bidValue < 0.01
            )
                ...
                throw globalFuncs.errorMsgs[31];
            else if (
                _objENS.revealObject &&
                _objENS.revealObject.name &&
                ens.normalise(_objENS.revealObject.name) != _objENS.name
            )
                throw globalFuncs.errorMsgs[34];
            else {
                if ($scope.objENS.status == $scope.ensModes.open)
                    $scope.openAndBidAuction();
                else if ($scope.objENS.status == $scope.ensModes.auction)
                    $scope.bidAuction();
                else if ($scope.objENS.status == $scope.ensModes.reveal)
                    $scope.revealBid();
            }
        } catch (e) {
