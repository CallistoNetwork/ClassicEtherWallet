...
var helpersCtrl = function($scope) {
    ...
    $scope.convertUnit = function(currentUnit) {
        unitNames.forEach(function(unit) {
            if (currentUnit !== unit) {
                $scope.units[unit] = $scope.units[currentUnit]
                    ? etherUnits.unitToUnit(
                          $scope.units[currentUnit],
                          currentUnit,
                          unit
                      )
                    : "";
    ...
    $scope.decimalToHex = function() {
        $scope.hexNumber = $scope.decimalNumber
            ? ethFuncs.decimalToHex($scope.decimalNumber)
            : "";
        $scope.hexToPaddedHex();
    };

    $scope.hexToDecimal = function() {
        $scope.decimalNumber = $scope.hexNumber
            ? ethFuncs.hexToDecimal($scope.hexNumber)
            : "";
        $scope.hexToPaddedHex();
    };

    $scope.hexToPaddedHex = function() {
        $scope.hexPaddedLeft = $scope.hexNumber
            ? ethFuncs.padLeft($scope.hexNumber, 64, "0")
            : "";
    };

    $scope.toSHA3 = function() {
        $scope.outputText = $scope.inputText
            ? ethUtil.sha3($scope.inputText).toString("hex")
            : "";
    };

    /* ENS STUFF */
    $scope.toEnsLabelHash = function() {
        $scope.ensLabelHash = $scope.ensLabel
            ? ENS.getSHA3($scope.ensLabel)
            : "";
        $scope.allTheThings();
    };

    $scope.toEnsSecretHash = function() {
        $scope.ensSecretHash = $scope.ensSecret
            ? ENS.getSHA3($scope.ensSecret.trim())
            : "";
        $scope.allTheThings();
    };

    $scope.toBidWei = function() {
        $scope.bidWei = $scope.bidEth
            ? Number(etherUnits.toWei($scope.bidEth, "ether"))
            : "";
        $scope.toBidHex();
    };

    $scope.toBidEth = function() {
        $scope.bidEth = $scope.bidWei
            ? Number(etherUnits.toEther($scope.bidWei, "wei"))
            : "";
        $scope.toBidHex();
    };

    $scope.toBidHex = function() {
        $scope.bidHex = $scope.bidWei
            ? ethFuncs.padLeft(ethFuncs.decimalToHex($scope.bidWei), 64, "0")
            : "";
        $scope.allTheThings();
    };
    ...
    $scope.getStartAuctionData = function() {
        $scope.startAuctionData = $scope.ensLabel
            ? ENS.getStartAuctionData($scope.ensLabel)
            : "";
    };
    ...
    $scope.getRevealBidData = function() {
        if ($scope.ensLabel && $scope.bidWei && $scope.ensSecret) {
            $scope.revealBidData = ENS.getRevealBidData(
                $scope.ensLabel,
                $scope.bidWei,
                $scope.ensSecret
            );
    ...
    $scope.getFinalizeAuctionData = function() {
        $scope.finalizeAuctionData = $scope.ensLabel
            ? ENS.getFinalizeAuctionData($scope.ensLabel)
            : "";
    };
    ...
    $scope.findMyPrivateKey = function() {
        ...
        basePrivateKey =
            basePrivateKey.substring(0, 2) == "0x"
                ? basePrivateKey.substring(2)
                : basePrivateKey;
        var characters = [
            "a",
            "b",
        ...
        for (
            var keyPosition = 0;
            keyPosition < basePrivateKey.length;
            keyPosition++
        ) {
            for (
                var character = 0;
                ...
                if (
                    ...
                    targetPublicAddress.toLowerCase()
                ) {
