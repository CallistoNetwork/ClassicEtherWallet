...
var offlineTxCtrl = function($scope, $sce, $rootScope, walletService) {
  ...
  $scope.setTokens = function() {
    ...
    for (var i = 0; i < $scope.tokens.length; i++) {
      $scope.tokenObjs.push(
        new Token(
          $scope.tokens[i].address,
          "",
          $scope.tokens[i].symbol,
          $scope.tokens[i].decimal,
          $scope.tokens[i].type
        )
      );
    }
  ...
  $scope.getWalletInfo = function() {
    if (ethFuncs.validateEtherAddress($scope.tx.from)) {
      ajaxReq.getTransactionData($scope.tx.from, function(data) {
        ...
        data = data.data;
        $scope.gasPriceDec = ethFuncs.hexToDecimal(
  ...
  $scope.validateAddress = function(address, status) {
    ...
    if (ethFuncs.validateEtherAddress(address)) {
      for (var i in CustomGasMessages) {
        if (
          $scope.tx.to.toLowerCase() == CustomGasMessages[i].to.toLowerCase()
        ) {
          $scope.customGasMsg =
            CustomGasMessages[i].msg != "" ? CustomGasMessages[i].msg : "";
        }
      }
      return true;
    } else {
  ...
  $scope.generateTx = function() {
    ...
    txData.gasPrice = ethFuncs.sanitizeHex(
      ethFuncs.decimalToHex($scope.gasPriceDec)
    );
    if ($scope.tokenTx.id != "ether") {
      txData.data = $scope.tokenObjs[$scope.tokenTx.id].getData(
        $scope.tx.to,
        $scope.tx.value
      ).data;
  ...
  $scope.confirmSendTx = function() {
    try {
      if ($scope.signedTx == "" || !ethFuncs.validateHexString($scope.signedTx))
        throw globalFuncs.errorMsgs[12];
      var eTx = new ethUtil.Tx($scope.signedTx);
      if (
        eTx.data.length &&
        Token.transferHex ==
          ethFuncs.sanitizeHex(eTx.data.toString("hex").substr(0, 8))
      ) {
        var token = Token.getTokenByAddress(
          ethFuncs.sanitizeHex(eTx.to.toString("hex"))
        );
        var decoded = ethUtil.solidityCoder.decodeParams(
          ["address", "uint256"],
          ethFuncs.sanitizeHex(eTx.data.toString("hex").substr(10))
        );
        $scope.tx.sendMode = "token";
        $scope.tokenTx.value = decoded[1]
          .div(new BigNumber(10).pow(token.decimal))
          .toString();
        $scope.tokenTx.to = decoded[0];
        $scope.unitReadable = token.symbol;
        $scope.tokenTx.from = ethFuncs.sanitizeHex(
          eTx.getSenderAddress().toString("hex")
        );
      } else {
        $scope.tx.sendMode = "ether";
        $scope.tx.value = eTx.value.length
          ? etherUnits.toEther(
              ethFuncs.sanitizeHex(eTx.value.toString("hex")),
              "wei"
            )
          : 0;
        $scope.unitReadable = ajaxReq.type;
        $scope.tx.from = ethFuncs.sanitizeHex(
          eTx.getSenderAddress().toString("hex")
        );
