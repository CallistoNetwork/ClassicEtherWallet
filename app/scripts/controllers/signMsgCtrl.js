...
var signMsgCtrl = function($scope, $sce, walletService) {
    ...
    $scope.generateSignedMsg = function() {
        try {
            ...
            if (typeof hwType != "undefined" && hwType == "web3") {
                ...
                $scope.notifier.info(
                    "Sent message for signing via ClassicMask / MetaMask / Mist."
                );
                ...
                var msg = ethUtil.hashPersonalMessage(
                    ethUtil.toBuffer(thisMessage)
                );
    ...
    $scope.verifySignedMessage = function() {
        ...
        if (typeof hwType != "undefined" && hwType == "trezor") {
            ...
            TrezorConnect.ethereumVerifyMessage(address, sig, message, function(
                response
            ) {
                if (response.success) {
