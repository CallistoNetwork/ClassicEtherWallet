...
var txStatusCtrl = function($scope) {
    ...
    var txToObject = function(tx) {
        ...
        if (tx) {
            ...
            if ($scope.txInfo.status == txStatus.found) {
                var _gasPrice = new BigNumber($scope.txInfo.gasPrice.wei).mul(
                    1.1
                );
                if (_gasPrice.lt(etherUnits.getValueOfUnit("gwei") * MIN_GAS))
                    _gasPrice = new BigNumber(
                        etherUnits.getValueOfUnit("gwei") * MIN_GAS
                    );
    ...
    $scope.checkTxStatus = function() {
        ...
        try {
            if (!Validator.isValidTxHash(txInfo.hash))
                throw globalFuncs.errorMsgs[36];
