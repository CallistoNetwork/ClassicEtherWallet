        ...
        link: function(scope, element, attrs) {
            ...
            scope.$watch("addressDrtv.ensAddressField", function() {
                ...
                if (scope.addressDrtv.ensAddressField === "") {
                    ...
                    Validator.isValidENSAddress(
                        scope.addressDrtv.ensAddressField
                    )
                ) {
                    _ens.getAddress(scope.addressDrtv.ensAddressField, function(
                        data
                    ) {
                    ...
                    scope.DexNSInterval = setTimeout(function() {
                        ...
                        var funcSig = ethFuncs.getFunctionSignature(
                            fullFuncName
                        );
                        var typeName = ethUtil.solidityUtils.extractTypeName(
                            fullFuncName
                        );
                        var types = typeName.split(",");
                        types = types[0] == "" ? [] : types;
                        var values = [];
                        curFunc.inputs[0].value =
                            scope.addressDrtv.ensAddressField;
                        for (var i in curFunc.inputs) {
                            if (curFunc.inputs[i].value) {
                                if (
                                    curFunc.inputs[i].type.indexOf("[") !==
                                        -1 &&
                                    curFunc.inputs[i].type.indexOf("]") !== -1
                                )
                                    values.push(
                                        curFunc.inputs[i].value.split(",")
                                    );
