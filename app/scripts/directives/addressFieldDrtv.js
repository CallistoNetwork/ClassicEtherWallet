    ...
    link: function(scope, element, attrs) {
      ...
      var placeholder =
        attrs.placeholder == undefined
          ...
          : attrs.placeholder;
      var labelTranslated =
        attrs.labeltranslated == undefined
          ? "SEND_addr"
          : attrs.labeltranslated;
      ...
      scope.addressDrtv = {
        ...
        ensAddressField:
          globalFuncs.urlGet("to") == null ? "" : globalFuncs.urlGet("to"),
        derivedAddress: "",
        readOnly: false
      };
      ...
      // placeholder +
      // "\" ng-model=\"addressDrtv.ensAddressField\" ng-disabled=\"addressDrtv.readOnly\" ng-class=\"Validator.isValidENSorEtherAddress(" +
      // varName +
      // ") ? 'is-valid' : 'is-invalid'\"/>\n \
      ...
      //          <div class=\"addressIdenticon\" title=\"Address Indenticon\" blockie-address=\"{{" +
      // varName +
      // "}}\" watch-var=\"" +
      // varName +
      // "\"></div>\n \
      ...
      scope.$watch("addressDrtv.ensAddressField", function() {
                ...
                if (scope.addressDrtv.ensAddressField === "") {
                    ...
                    scope.addressDrtv.ensAddressField.length == 42 &&
          scope.addressDrtv.ensAddressField[0] == "0" &&
          scope.addressDrtv.ensAddressField[1] == "x"
                ) {
                    console.log("GRC!");
                    if (Validator.isValidAddress(scope.addressDrtv.ensAddressField)) {
                        setValue(scope.addressDrtv.ensAddressField);
                        if (
                            !Validator.isChecksumAddress(scope.addressDrtv.ensAddressField)
                        ) {
                            scope.notifier.info(globalFuncs.errorMsgs[35]);
                        }
                    }
                } else if (
                    Validator.isValidENSAddress(scope.addressDrtv.ensAddressField)
                ) {
              ...
              data.data == "0x"
                            ...
                            scope.addressDrtv.derivedAddress =
                "0x0000000000000000000000000000000000000000";
                            scope.addressDrtv.showDerivedAddress = true;
                        } else {
                            setValue(data.data);
                            scope.addressDrtv.derivedAddress = ethUtil.toChecksumAddress(
                                data.data
                            );
                            scope.addressDrtv.showDerivedAddress = true;
                        }
          ...
          !(
              scope.addressDrtv.ensAddressField == 42 &&
            scope.addressDrtv.ensAddressField[0] == "0" &&
            scope.addressDrtv.ensAddressField[1] == "x"
          )
                ) {
              ...
              "0x" +
              funcSig +
              ethUtil.solidityCoder.encodeParams(types, values);
                  ...
                  data.data === "0x"
                                    ...
                                    setValue("null");
                                    scope.addressDrtv.derivedAddress =
                    "    ⊘ ERROR: DexNS Name is not found!";
                                    scope.addressDrtv.showDerivedAddress = true;
                                } else {
                                    setValue(data.data);
                                    // found dexns registed name, set address, and symbol and decimals will be called.
                                    scope.addressDrtv.ensAddressField = ethUtil.toChecksumAddress(
                                        data.data
                                    );
