...
ens.prototype.getOwner = function(name, callback) {
    this.getOwnerResolverAddress(
        this.registryABI.owner,
...
ens.prototype.getAddress = function(name, callback) {
    ...
    _this.getResolver(name, function(data) {
        if (data.error) callback(data);
            ...
            _this.getOwnerResolverAddress(
                _this.resolverABI.addr,
                data.data,
                name,
                callback
            );
            ...
            ajaxReq.getEthCall(
                ...
                function(data) {
                    if (data.error || data.data == "0x") callback(data);
                        ...
                        var outTypes = _this.resolverABI.name.outputs.map(
                            function(i) {
                                return i.type;
                            }
                        );
...
var isSecretHashed = function(secret) {
    return (
        secret.substring(0, 2) == "0x" &&
        secret.length == 66 &&
        Validator.isValidHex(secret)
    );
};
    ...
    ajaxReq.getEthCall(
        ...
        function(data) {
            if (data.error) callback(data);
                ...
                var res = ethUtil.solidityCoder.decodeParams(
                    outTypes,
                    data.data.replace("0x", "")
                );
                data.data = {
