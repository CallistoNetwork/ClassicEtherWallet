...
var DigitalBitboxEth = function(comm, sec) {
    ...
    this.key = Crypto.createHash("sha256")
        .update(new Buffer(DigitalBitboxEth.sec, "ascii"))
        .digest();
    this.key = Crypto.createHash("sha256")
        .update(this.key)
        .digest();
...
DigitalBitboxEth.parseError = function(errObject) {
    var errMsg = {
        err101:
            'The Digital Bitbox is not initialized. First use the <a href="https://digitalbitbox.com/start" target="_blank" rel="noopener">Digital Bitbox desktop app</a> to set up a wallet.', // No password set
        err250:
            'The Digital Bitbox is not initialized. First use the <a href="https://digitalbitbox.com/start" target="_blank" rel="noopener">Digital Bitbox desktop app</a> to set up a wallet.', // Wallet not seeded
        err251:
            'The Digital Bitbox is not initialized. First use the <a href="https://digitalbitbox.com/start" target="_blank" rel="noopener">Digital Bitbox desktop app</a> to set up a wallet.', // Wallet not seeded
        err109:
        ...
        if (typeof error != "undefined") {
            ...
            try {
                ...
                if ("error" in response) {
                    callback(
                        undefined,
                        DigitalBitboxEth.parseError(response.error)
                    );
                    return;
                }
            ...
            try {
                ...
                if ("ciphertext" in response) {
                    ...
                    if ("echo" in response) {
                        ...
                        var cmd = '{"sign":""}';
                        cmd = DigitalBitboxEth.aes_cbc_b64_encrypt(
                            cmd,
                            self.key
                        );
                        self.comm.exchange(cmd, localCallback);
