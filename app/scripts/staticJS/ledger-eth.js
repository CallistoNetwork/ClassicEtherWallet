...
LedgerEth.prototype.signTransaction = function(path, rawTxHex, callback) {
    ...
    while (offset != rawTx.length) {
        ...
        var buffer = new Buffer(
            offset == 0
                ? 5 + 1 + splitPath.length * 4 + chunkSize
                : 5 + chunkSize
        );
        buffer[0] = 0xe0;
        buffer[1] = 0x04;
        ...
        buffer[3] = 0x00;
        buffer[4] =
            offset == 0 ? 1 + splitPath.length * 4 + chunkSize : chunkSize;
        if (offset == 0) {
            buffer[5] = splitPath.length;
            splitPath.forEach(function(element, index) {
                buffer.writeUInt32BE(element, 6 + 4 * index);
...
LedgerEth.prototype.signPersonalMessage_async = function(
    ...
    while (offset != message.length) {
        var maxChunkSize =
            offset == 0 ? 150 - 1 - splitPath.length * 4 - 4 : 150;
        var chunkSize =
        ...
        var buffer = new Buffer(
            offset == 0
                ? 5 + 1 + splitPath.length * 4 + 4 + chunkSize
                : 5 + chunkSize
        );
        buffer[0] = 0xe0;
        buffer[1] = 0x08;
